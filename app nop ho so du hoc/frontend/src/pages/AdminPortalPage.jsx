  ...
  return (
    <div className="ap-root">
      <style>{`
        ...
        .ap-item { width: 100%; height: 38px; border: none; background: transparent; border-radius: 11px; display: flex; align-items: center; gap: 10px; padding: 0 10px; color: #4f6386; font-size: 14px; cursor: pointer; text-align: left; }
      ...
      <aside className="ap-side">
        <div>
          ...
          <div className="ap-brand-sub">Hệ thống du học</div>
        </div>
        <div className="ap-profile">
          <div className="ap-avatar">{adminInitials}</div>
          <div>
            <div className="ap-name">{adminDisplayName}</div>
            <div className="ap-role">Tài khoản quản trị viên</div>
          </div>
        </div>
        <div className="ap-menu">
          <button type="button" className="ap-item"><LayoutDashboard size={17} /> Bảng điều khiển</button>
          <button
            type="button"
            className={`ap-item ${activeSection === "students" ? "active" : ""}`}
            onClick={() => setActiveSection("students")}
          >
            <Users size={17} /> Hồ sơ học sinh
          </button>
          <button
            type="button"
            className={`ap-item ${activeSection === "security" ? "active" : ""}`}
            onClick={() => setActiveSection("security")}
          >
            <ShieldCheck size={17} /> Bảo mật & Phân quyền
          </button>
          <button type="button" className="ap-item"><FileText size={17} /> Đơn ứng tuyển</button>
          <button type="button" className="ap-item"><FolderOpen size={17} /> Tài liệu</button>
          <button type="button" className="ap-item"><Settings size={17} /> Cài đặt</button>
        </div>
        <button className="ap-logout" onClick={onLogout}><LogOut size={17} /> Đăng xuất</button>
      </aside>
      ...
      <main className="ap-main">
        ...
        <section className="ap-top">
          <div className="ap-head">
            <div>
              <h1 className="ap-title">Quản lý Sinh n</h1>
        ...
        {showDetail && selectedStudent && (
          ...
          <SecurityCenterPage title="Trung tam Bao mat" apiBaseUrl={`${API_BASE}/api/security`} />
        )}
      </main>
    </div>
  );
